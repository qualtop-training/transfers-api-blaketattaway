...
func NewTransfersMySQLRepository(cfg config.MySQLDB) *TransfersMySQLRepo {
	...
	if err := db.Ping(); err != nil {
		panic(fmt.Sprintf("error pinging MySQL: %v", err))
	}
...
func (r *TransfersMySQLRepo) Create(ctx context.Context, transfer models.Transfer) (string, error) {
	...
	res, err := r.db.ExecContext(ctx, query,
		dao.SenderID, dao.ReceiverID, dao.Currency, dao.Amount, dao.State)
...
func (r *TransfersMySQLRepo) GetByID(ctx context.Context, id string) (models.Transfer, error) {
	...
	err := r.db.QueryRowContext(ctx, query, id).Scan(&dao.ID, &dao.SenderID, &dao.ReceiverID, &dao.Currency, &dao.Amount, &dao.State)

	if err != nil {

		if err == sql.ErrNoRows {
			return models.Transfer{}, fmt.Errorf("transfer with ID %s not found: %w", id, known_errors.ErrNotFound)
		}

		return models.Transfer{}, fmt.Errorf("error fetching transfer: %w", err)
	}
...
func (r *TransfersMySQLRepo) GetAll(ctx context.Context) ([]models.Transfer, error) {
	...
	for rows.Next() {
		var dao transferMySQLDAO
		if err := rows.Scan(&dao.ID, &dao.SenderID, &dao.ReceiverID, &dao.Currency, &dao.Amount, &dao.State); err != nil {
...
func (r *TransfersMySQLRepo) GetBySenderID(ctx context.Context, senderID string) ([]models.Transfer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error fetching transfers by sender ID: %w", err)
	}
	defer rows.Close()
...
func (r *TransfersMySQLRepo) GetByReceiverID(ctx context.Context, receiverID string) ([]models.Transfer, error) {
	...
	query := `SELECT id, sender_id, receiver_id, currency, amount, state FROM transfers WHERE receiver_id = ?`
	rows, err := r.db.QueryContext(ctx, query, receiverID)
	if err != nil {
		return nil, fmt.Errorf("error fetching transfers by receiver ID: %w", err)
	}
	defer rows.Close()
